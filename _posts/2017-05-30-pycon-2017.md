...
recently began formulating a new kind of free class for new programmers that
relies completely on self-guided tutorials. The benefit of this is that people
do not get driven away by the class going either too fast or too slow; by its
nature it adapts to the learning experience of each individual. In addition,
once anyone has solved one of the steps, they can help their classmates solve
that step (indeed, they are probably better at it, being closer to "beginner's
mind").

The site is [EveryoneProgram.com](www.EveryoneProgram.com) and might still be
very rough when you look at it, but the content is basically there, including
...
discuss it. Others in the session discussed similar experiences.

I have not found similar discussions at other conferences. One of the unique
things about Pycon is that it (intentionally) has a very high percentage of
women attendees and speakers. Guido's stated goal is 50% and it seems like
that number is fairly close in terms of speakers, and I might guess the women
attendees could be as high as 35% or more. Minorities of various kinds are
also becoming more represented.

Because of this, it seems like people are more comfortable having these kinds
of discussions.
...
1. **Less is More**. The more words you have, the more work your reader must do.
Shorter sentences are easier to read and understand. They also require more work.
Multiple editing passes are usually required.

2. **Active Voice**. You can find some examples [here](http://examples.yourdictionary.com/examples-of-active-and-passive-voice.html), although these are rather basic. Perhaps
we need another category of "super-active" or "direct" voice, as it's usually possible
to make your sentences even more immediate -- although it takes even more work to get there.

Active voice helps a lot with point `1.`, because active sentences are almost
always shorter than passive sentences.

3. **Read It Aloud**. This is a trick I learned from a friend many years ago. When
you read silently, your mind tends to skip and skim. When you read *every single word*
out loud, it forces you to really examine your prose. Problems jump out at you and
you're surprised that you didn't see such glaring errors before. It takes a lot of
time (especially for book chapters) but it pays off in major ways.
...
Frederic Laloux says that these topics are different from one Teal
...
just use them as a starting point. I think that I can make an important
...
So much of my effort was spent on learning the Toga architecture and figuring
out how to add menus for Windows apps. This was tremendously educational
but I'm still not close to starting to create my decorators for menus. However,
I'd rather continue working on Beeware until it gets to the point where I can,
rather than building something from scratch. This way I'll produce something
...
experiences. When you install a package you want to modify and test, it turns
out that you must make it editable using the `-e` flag, as in `pip install
-e`. Otherwise your changes won't be seen and used, and only the original
installation will ever be seen when you run your program. Without knowing this
I experienced some frustration until Russel walked me through it. If I hadn't
...
Google Keep. I used to make notes and then set them aside and forget them, but
...
- A practice during discussions: "step up/step back" -- if you normally speak a
  lot, speak less; if you normally don’t talk, speak more.
